...
function calculateStatus(date, workingDays, shiftType) {
  ...
  // تحويل workingDays إلى مصفوفة بناءً على القيمة ونوع الدوام
  let workDaysArray;
  if (shiftType === '24/24') {
    // للدوام 24/24، جميع الأيام تعتبر أيام عمل
    workDaysArray = daysOfWeek;
  } else {
    // للدوام الإداري أو أنواع الدوام الأخرى
    workDaysArray = workingDays === '5' 
      ? ['الأحد', 'الإثنين', 'الثلاثاء', 'الأربعاء', 'الخميس']
      : ['الأحد', 'الإثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة'];
  }

  return workDaysArray.includes(day) ? 'absent' : 'weekly_off';
}
